...
The `webhook` source normalizes string severity values to lowercase, while the `alerts` (polling) source reports the integer `cf.eventSeverity` (higher is more severe).
...
Create a vault-encrypted file to store your credentials securely:
...
When HMAC verification is enabled (set `token:` on the source), sign the raw body and send it as either `X-Hub-Signature-256: sha256=<hex>` or the alternative `X-Edwin-Signature: <hex>`.
...
1. In the AAP UI, navigate to **Automation Decisions → Projects → Create Project**, and set the Source Control URL to this repository.
